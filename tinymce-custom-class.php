...
class TinyMCE_Custom_Class {
	...
	*/
	...
	function add_tinymce_plugin( $plugin_array ) {
	    ...
	    $plugin_array['custom_footnote'] = plugin_dir_url( __FILE__ ) . 'tinymce-custom-class.js';
	    return $plugin_array;
	...
	function add_tinymce_toolbar_button( $buttons ) {
	    ...
	    array_push( $buttons, 'custom_footnote' );
	    return $buttons;
...
function admin_footer_scripts() {
	...
	<script type="text/javascript">
		QTags.addButton( 'custom_footnote', 'Insert Custom Class', insert_custom_class );
		function insert_custom_class() {
		    ...
		    var result = prompt('Enter the foot note', title);
		    if ( !result ) {
		        // User cancelled - exit
		        return;
		    }
		    ...
		    // Insert
		    QTags.insertContent(' <a class="footnote-text" title="' + result + '" style="text-decoration:underline;background-color:#F4FA58;">' + text + '</a>');
		}
	</script>
	<?php
